...
{
	public:
		char get_char(ulint c, vector<char>& heads, vector<ulint>& lens)
		{
			if(c != i){ cerr << "error!" << endl; exit(1); }
			if(j==0)
			{
				j = lens[x];
				h = heads[x++];
...
public:
	...
	rle_string(vector<char>& heads, vector<ulint>& lens, ulint input_size, ulint B = 2)
	{
		...
		{
			for(ulint j=0;j<lens[i]-1;++j)
			{
				runs_bv.push_back(false);
				runs_per_letter_bv[heads[i]].push_back(false);
			}

			run_heads_s.push_back(heads[i]);
